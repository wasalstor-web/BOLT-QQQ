...
export const meta: MetaFunction = () => {
  return [
    { title: 'تسجيل الدخول - مبسط إديتر' },
    { name: 'description', content: 'سجل دخولك للوصول لمشاريعك' },
  ];
};
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  return json({ error: 'البريد الإلكتروني أو كلمة المرور غير صحيحة' }, { status: 400 });
}
      ...
      >
        ...
        <div className="bg-slate-900/80 backdrop-blur-xl rounded-3xl border border-gray-800 p-8 shadow-2xl">
          <form onSubmit={handleSubmit} className="space-y-6">
            {(authError || urlError || actionData?.error) && (
              <motion.div
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 0 }}
                className="p-4 bg-red-500/20 border border-red-500/30 rounded-xl text-red-300 text-sm"
              >
                {authError || (urlError === 'auth_failed' ? 'فشل تسجيل الدخول' : actionData?.error)}
              </motion.div>
            )}
