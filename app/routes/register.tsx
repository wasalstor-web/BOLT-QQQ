...
function getPasswordStrength(password: string): { score: number; label: string; color: string } {
  ...
  if (score <= 1) { return { score: 20, label: 'ضعيفة جداً', color: 'bg-red-500' }; }
  if (score === 2) { return { score: 40, label: 'ضعيفة', color: 'bg-orange-500' }; }
  if (score === 3) { return { score: 60, label: 'متوسطة', color: 'bg-yellow-500' }; }
  if (score === 4) { return { score: 80, label: 'قوية', color: 'bg-green-500' }; }
  return { score: 100, label: 'قوية جداً', color: 'bg-emerald-500' };
}

export const meta: MetaFunction = () => {
  return [
    { title: 'إنشاء حساب - مبسط إديتر' },
    { name: 'description', content: 'أنشئ حسابك المجاني وابدأ بناء مواقعك' },
  ];
};
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  return json({ error: 'يرجى ملء جميع الحقول' }, { status: 400 });
}
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!agreedToTerms) {
      setAuthError('يرجى الموافقة على شروط الاستخدام');
      setIsLoading(false);
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950 text-white flex items-center justify-center p-6" dir="rtl">
      {/* Animated Background */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-40 -left-40 w-80 h-80 bg-pink-500/30 rounded-full blur-[100px] animate-pulse" />
        <div className="absolute bottom-20 -right-40 w-96 h-96 bg-purple-500/20 rounded-full blur-[120px] animate-pulse delay-1000" />
      </div>
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold mt-6 mb-2">إنشاء حساب جديد</h1>
          <p className="text-gray-400">ابدأ رحلتك في بناء المواقع</p>
        </div>

        {/* Register Form */}
        <div className="bg-slate-900/80 backdrop-blur-xl rounded-3xl border border-gray-800 p-8 shadow-2xl">
          <form onSubmit={handleSubmit} className="space-y-5">
            {(authError || actionData?.error) && (
              <motion.div
            ...
            {successMessage && (
              <motion.div
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 0 }}
                className="p-4 bg-green-500/20 border border-green-500/30 rounded-xl text-green-300 text-sm"
              >
                {successMessage}
              </motion.div>
            )}
            ...
            <div>
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={`text-xs font-medium ${
                      ...
                      passwordStrength.score <= 60 ? 'text-yellow-400' : 'text-green-400'
                    }`}>
              ...
              <p className="mt-2 text-xs text-gray-500">
                يجب أن تحتوي على 8 أحرف على الأقل مع أرقام وحروف كبيرة
              </p>
            </div>
            ...
            <div>
              <label className="flex items-start gap-3 cursor-pointer">
                <input
                  ...
                  className="w-5 h-5 mt-0.5 rounded border-gray-600 bg-slate-800 text-purple-500 focus:ring-purple-500"
                />
          ...
          <div className="mt-8">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-4 bg-slate-900 text-gray-400">أو سجل عبر</span>
              </div>
            </div>
        ...
        <div className="mt-8 grid grid-cols-3 gap-4 text-center">
          {[
            ...
            <motion.div
              ...
              className="p-4 bg-slate-900/50 rounded-xl border border-gray-800"
            >
