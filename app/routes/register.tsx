...
function getPasswordStrength(password: string): { score: number; label: string; color: string } {
  ...
  if (score <= 1) {
    return { score: 20, label: 'ضعيفة جداً', color: 'bg-red-500' };
  }

  if (score === 2) {
    return { score: 40, label: 'ضعيفة', color: 'bg-orange-500' };
  }

  if (score === 3) {
    return { score: 60, label: 'متوسطة', color: 'bg-yellow-500' };
  }

  if (score === 4) {
    return { score: 80, label: 'قوية', color: 'bg-green-500' };
  }
        ...
        <div className="bg-slate-900/80 backdrop-blur-xl rounded-3xl border border-gray-800 p-8 shadow-2xl">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <p className="mt-2 text-xs text-gray-500">يجب أن تحتوي على 8 أحرف على الأقل مع أرقام وحروف كبيرة</p>
            </div>
